...
int ch;
 for(;;) {
    ...
    if (change){
       ...
       if (light & 1)
          print_uart0("North Green / Red East\n");
       else if (light & 2)
          print_uart0("North Yellow / Red East\n");
       else if (light & 8)
          print_uart0("North Red / Green East\n");
       else if (light & 16)
          print_uart0("North Red / Yellow East\n");
 ...
 //  emulate  Input = SENSOR;     // read sensors
   if ((UART0->FR & RXFE) == 0) {	// A key is pressed
      ...
      change =1;
      ch = UART0->DR;	// ch = key that was entered
